...
def issuer_url(issuer_id: str) ->  str:
    return CLIENT_CONFIG[issuer_id]['issuer']


class TestRPHandler(object):
    @pytest.fixture(autouse=True)
    def rphandler_setup(self):
        self.rph1 = RPHandler(BASE_URL, client_configs=CLIENT_CONFIG,
                              keyjar=CLI_KEY, module_dirs=['oidc'])
        self.rph2 = RPHandler(base_url=BASE_URL, client_configs=CLIENT_CONFIG,
                              keyjar=CLI_KEY, module_dirs=['oidc'])

    def test_begin(self):
        req_info = self.rph1.begin(issuer_id='github')
        assert set(req_info.keys()) == {'url', 'request', 'method'}
        _handler_state = self.rph1.dump()
        # down and up again
        self.rph2.load(_handler_state)
        _github_url = issuer_url('github')
        client = self.rph2.find_client_by_issuer_url(_github_url)
        assert client.service_context.get('issuer') == _github_url

    def test_finalize_auth(self):
        req_info = self.rph1.begin(issuer_id='linkedin')
        ...
        auth_response = AuthorizationResponse(code='access_code',
                                              state=req_info["request"]['state'])
        _authn_resp = auth_response.to_dict()

        # the authorization response arrives at a callback URL
        # emulating that here
        cb_url = req_info["request"]['redirect_uri']

        client = self.rph2.find_client_by_redirect_uri(cb_url)
        resp = client.finalize_auth(response=_authn_resp, issuer=client.service_context.issuer)
        assert set(resp.keys()) == {'state', 'code'}
        aresp = client.service['authorization'].service_context.state.get_item(
            AuthorizationResponse, 'auth_response', req_info["request"]['state'])
        assert set(aresp.keys()) == {'state', 'code'}

    def test_get_client_authn_method(self):
        res = self.rph1.begin(issuer_id='github')
        _session = self.rph1.get_session_information(res['request']['state'])
        client = self.rph1.find_client_by_issuer_url(_session['iss'])
        authn_method = client.get_client_authn_method('token_endpoint')
        assert authn_method == ''
        ...
        res = self.rph2.begin(issuer_id='linkedin')
        _session = self.rph2.get_session_information(res['request']['state'])
        client = self.rph2.find_client_by_issuer_url(_session['iss'])
        authn_method = client.get_client_authn_method('token_endpoint')
        assert authn_method == 'client_secret_post'

        _handler_state = self.rph2.dump()
        # Sync 2 and 1
        self.rph1.load(_handler_state)

        # Now they should be in sync
        _issuers = [client.service_context.issuer for client in self.rph1.clients]
        assert set(_issuers) == {'https://github.com/login/oauth/authorize',
                                 'https://www.linkedin.com/oauth/v2/'}
        _issuer_ids = [client.issuer_id for client in self.rph2.clients]
        assert set(_issuer_ids) == {'github','linkedin'}
...
#                                         '__expires_at'}
#
#             atresp = client.service['accesstoken'].service_context.state.get_item(
